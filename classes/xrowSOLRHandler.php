...
class xrowSOLRHandler extends ezcSearchSolrHandler
{
    const MS = 8;
     ...
     * Connects to Solr
     *
     * @throws ezcSearchCanNotConnectException if a connection can not be
     *         established.
     */
    ...
    private function getLine( $maxLength = false )
    {
        ...
        while ( strpos( $line, "\n" ) === false )
        {
            ...
            /*
             * If solr aborts the connection, fgets() will return false. We need
             * to throw an exception here to prevent the calling code from
    ...
    /**
     * Executes a search by building and sending a query and returns the raw
     * result
     *
     * @param string $queryWord            
     * @param string $defaultField            
    ...
    /**
     * Creates a search query object with the fields from the definition filled
     * in.
     *
     * @param string $type            
    ...
    /**
     * Returns the field name as used by solr created from the field $name and
     * $type.
     *
     * @param string $name            
     * @param string $type            
     ...
     *
     * Depending on the $fieldType the $value is modified so that the indexer
    ...
    /**
     * This method prepares a value or an array of $values before it is passed
     ...
     *         submitted, but the field has not been defined as a multi-value
     *         field.
     *        
    ...
    public function mapFieldValuesForIndex( $field, $values )
    {
        ...
        if( $field->type === ezcSearchDocumentDefinition::BOOLEAN )
        {
            return $values[0];
        }
        return $values;
    ...
    /**
     * This method prepares a value or an array of $values after it has been
     * returned by search handler.
     *
    ...
    {
        $r = $this->sendRawPostCommand( 'update', array( 
            'wt' => 'json' 
        ), '<commit/>' );
    }
    ...
    public function index( ezcSearchDocumentDefinition $definition, $document )
    {
        ...
        $xml->writeAttribute( 'name', 'meta_anon_access_b' );
        $xml->text( 'true' );
        $xml->endElement();
        
        $xml->startElement( 'field' );
        $xml->writeAttribute( 'name', 'meta_is_invisible_b' );
        $xml->text( 'false' );
        $xml->endElement();
        
        $xml->startElement( 'field' );
        $xml->writeAttribute( 'name', 'meta_path_si' );
        $xml->text( '375849' );
        $xml->endElement();
        ...
        /*
         * if ( $this->inTransaction == 0 ) { $this->runCommit(); }
         */
    }

    /**
     * Creates a delete query object with the fields from the definition filled
     * in.
     *
     * @param string $type            
     * @param ezcSearchDocumentDefinition $definition            
    ...
    { 
        $queryString = "<delete><query>{$url}</query></delete>";
        $this->sendRawPostCommand( 'update', null, $queryString );
    }
     ...
     *
     * @throws ezcSearchQueryException if the delete query failed.
     *        
     ...
     *
     * @throws ezcSearchIdNotFoundException if the document with ID $id did not
     *         exist.
     *        
     * @param mixed $id            
     * @param ezcSearchDocumentDefinition $definition            
